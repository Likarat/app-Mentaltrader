...
- [ ] 1.1 `OperationDao`: agregar query paginada `pagingSourceOrderedByDateDesc(): PagingSource<Int, Operation>` (sin filtros todavía, esos llegan en EP-003-e)
- [ ] 1.2 Dependencia `androidx.paging:paging-compose` + `androidx.paging:paging-runtime` (ya en `stack-allowlist.json`) — agregar a `libs.versions.toml`/`build.gradle.kts`
- [ ] 1.3 `HistorialListItem` (sealed interface: `MonthHeader`, `DayHeader`, `OperationRow`) + función de mapeo `Operation -> HistorialListItem` con `PagingData.insertSeparators`
- [ ] 1.4 `HistorialViewModel` nuevo: `Pager(PagingConfig(pageSize = ...)) { operationDao.pagingSourceOrderedByDateDesc() }.flow.cachedIn(viewModelScope)`, transformado a `Flow<PagingData<HistorialListItem>>`
- [ ] 1.5 `HistorialScreen`: reemplazar la `LazyColumn` de verificación por `LazyPagingItems` (`collectAsLazyPagingItems`) renderizando `MonthHeader`/`DayHeader`/`OperationRow` según el tipo, con la tarjeta compacta (miniatura o ícono genérico, Activo+Hora arriba, Resultado+R coloreado abajo)
- [ ] 1.6 Ícono genérico para operaciones sin imagen (evitar espacio vacío en la tarjeta)
- [ ] 1.7 Tests unitarios: `HistorialViewModelTest` con fake `OperationDao`/`PagingSource` — mapeo a `HistorialListItem` inserta separadores correctos (mes más reciente arriba, día dentro de mes, orden descendente)
- [ ] 1.8 Test instrumentado con dataset sintético grande (3,000-5,000 operaciones sembradas vía `operationDao.insert` en loop, sin UI) verificando que `pagingSourceOrderedByDateDesc` no carga todo a memoria (tamaño de página real) y que el scroll llega hasta el final sin excepciones
- [ ] 1.9 `journey_smoke` EP-003-a: abrir Historial con operaciones reales de distintos meses, ver el agrupamiento y la tarjeta compacta reales
...
- [ ] 3.1 `OperationDao.update(operation: Operation)` (Room `@Update`)
- [ ] 3.2 `OperationDao`: `deleteById(id: Long)` + reutilizar `OperationImageDao.getByOperationId`/borrado de archivos (mismo patrón de limpieza que `DataResetService`) para el borrado físico definitivo
- [ ] 3.3 `OperationFormViewModel`: parámetro opcional `editingOperationId: Long? = null`; si no es null, `init` carga la operación existente y prellena el estado (en vez del prellenado de HU-005); `save()` llama `update(...)` en vez de `insert(...)` en modo edición
- [ ] 3.4 Botón "Editar" en `HistorialDetalleScreen` navega a `OperationFormScreen` con `editingOperationId` seteado
- [ ] 3.5 `HistorialViewModel`: `pendingDeleteIds: StateFlow<Set<Long>>` + `onRequestDelete(id)` (agrega el id, lanza `Job` con `delay(UNDO_WINDOW_MS)` que ejecuta el borrado físico real si no se canceló) + `onUndoDelete(id)` (cancela el `Job`, quita el id) — el `PagingData` se filtra excluyendo `pendingDeleteIds`
- [ ] 3.6 Snackbar "Deshacer" en `HistorialDetalleScreen`/`HistorialScreen` tras confirmar eliminación, vuelve a Historial al confirmar
- [ ] 3.7 Tests unitarios: `OperationFormViewModelTest` (modo edición: prellenado correcto, `update` en vez de `insert`, validación de HU-004 aplica igual) + `HistorialViewModelTest` (pendingDeleteIds oculta el id, deshacer lo restaura sin tocar Room, expirar el delay borra de verdad incluyendo imágenes)
- [ ] 3.8 Test instrumentado: editar una operación real desde el detalle y ver el cambio reflejado en el listado + detalle; eliminar una real, deshacer y verificar que sigue intacta; eliminar una real, esperar el timeout y verificar que la fila y sus archivos de imagen ya no existen
- [ ] 3.9 `journey_smoke` EP-003-c: editar una operación real y ver el cambio reflejado; eliminar una y deshacer justo a tiempo
