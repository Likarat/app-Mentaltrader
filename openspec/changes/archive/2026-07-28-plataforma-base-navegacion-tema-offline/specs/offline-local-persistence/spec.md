...
> **Nota de alcance (revisada durante la implementación de este change)**: Room exige al menos una entidad (`@Database` no admite `entities = []` — falla en tiempo de compilación de KSP: "must specify list of entities"). Las entidades reales (`Operation`, `CatalogItem`, `OperationImage`) pertenecen a EP-001. Por lo tanto, esta Requirement queda **declarada pero no implementada** en este change — la implementación real (con `AppDatabase` y su primera entidad) se entrega en el change de EP-001, que deberá incluir un delta `MODIFIED Requirements` aquí marcando estos escenarios como satisfechos. No se crea ninguna entidad "dummy" en este slice solo para cerrar el gate artificialmente.
