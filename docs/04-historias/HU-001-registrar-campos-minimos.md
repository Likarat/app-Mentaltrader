...
quiero **registrar una operación con sus campos identificadores mínimos (Fecha, Hora, Activo, Dirección y Resultado), más los campos opcionales de contexto del resultado (Riesgo %, Resultado en R y Ratio planeado)**,
...
**Fix adicional durante la implementación (2026-07-28)**: se detectó, al construir el modelo `Operation` (spec §3.1), que `plannedRatio` ("Ratio planeado", texto corto opcional, ej. "1:2") tampoco tenía ninguna historia que lo creara en el formulario. Se agrega aquí, junto a Riesgo %/Resultado en R, por ser del mismo tipo (campo de texto corto opcional, sin lógica de validación propia) — no amerita una historia nueva ni un escenario adicional (ya se cubre dentro de los Escenarios 4 y 5 existentes).
...
### Escenario 4 — Edge: guardar con los campos opcionales de Riesgo (%), Resultado en R y Ratio planeado completados
- **Dado que** el usuario completó los 5 campos mínimos y además ingresó un valor en Riesgo (%), en Resultado en R y en Ratio planeado (ej. "1:2")
- **Cuando** presiona "Guardar"
- **Entonces** el sistema almacena los tres valores junto con el resto de la operación, dejando la validación de rango de Riesgo/R a cargo de HU-004

### Escenario 5 — Edge: guardar dejando Riesgo (%), Resultado en R y Ratio planeado vacíos
- **Dado que** el usuario completó los 5 campos mínimos pero dejó vacíos Riesgo (%), Resultado en R y Ratio planeado (los tres opcionales)
- **Cuando** presiona "Guardar"
- **Entonces** el sistema guarda la operación con normalidad, dejando los tres campos como nulos

## Notas técnicas (opcional)

- Alcance: 5 campos identificadores mínimos + 3 campos opcionales (Riesgo %, Resultado en R, Ratio planeado) — 8 de los campos cubiertos por EP-001 fuera de HU-002/HU-003. Calidad, Emoción antes/después, Error se cubren en HU-002; Descripción entrada en HU-003; los 3 campos de motivo en texto libre (`emotionBeforeReason`, `emotionAfterReason`, `errorReason`) se cubren en HU-002 (van junto a sus respectivas selecciones de catálogo).
...
- [x] **E**stimable — alcance acotado a 8 campos con tipos de control bien conocidos (fecha, hora, 2 selects, enum, 2 decimales, 1 texto corto); el equipo puede estimar con confianza.
- [x] **S**mall — alcance ampliado levemente (3 campos opcionales más) pero sigue cabiendo razonablemente en 2-4 días; los tres son simples inputs sin lógica de negocio propia (la validación de Riesgo/R vive en HU-004; Ratio planeado no tiene validación de formato).
